#!/bin/bash
#
# Antigravity CLI (agy) - Unix Bootstrapper Script (Bash/Zsh/Fish)
# VENDORED COPY
#
# Source: https://antigravity.google/cli/install.sh
# Vendored: 2026-05-28 — refresh periodically.
#
# Why vendored: the upstream URL antigravity.google requires IPv6, which
# Docker containers on macOS lack by default (Docker Desktop / OrbStack
# both NAT'd IPv4-only). Build-time `curl | bash` therefore times out
# even though the binary CDN (Cloud Run, IPv4-reachable) below works.
#
# Refresh procedure:
#   curl -fsSL https://antigravity.google/cli/install.sh -o install-agy.sh
#   (preserve this header block, replace the rest)
#
